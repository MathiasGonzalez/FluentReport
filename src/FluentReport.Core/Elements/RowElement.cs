...
public class RowElement : ElementBase
{
    ...
    public override Size Measure(MeasureContext context)
    {
        ...
        foreach (var item in Items)
        {
            ...
            var s = item.Element?.Measure(context.WithDimensions(width, context.AvailableHeight)) ?? Size.Zero;
    ...
    public override void Render(RenderContext context, Position position, Size size)
    {
        ...
        foreach (var item in Items)
        {
            ...
            var s = item.Element?.Measure(context.MeasureContextFor(width, size.Height)) ?? Size.Zero;
