...
public class ChartBuilder(ChartElement chart, ContainerBuilder parent)
{
    /// <summary>Sets the chart type (bar or line).</summary>
    public ChartBuilder Type(ChartType type) { chart.Type = type; return this; }

    /// <summary>Sets an optional title drawn above the chart.</summary>
    public ChartBuilder Title(string title) { chart.Title = title; return this; }

    /// <summary>Sets the fixed height of the chart in points. Default is 200.</summary>
    public ChartBuilder Height(float height) { chart.FixedHeight = height; return this; }

    /// <summary>Sets the category (X-axis) labels.</summary>
    public ChartBuilder Categories(IEnumerable<string> labels)
    {
        chart.CategoryLabels.Clear();
        chart.CategoryLabels.AddRange(labels);
        return this;
    }
    ...
    public ContainerBuilder Padding(float all) => parent.Padding(all);

    /// <inheritdoc cref="ContainerBuilder.Background(string)"/>
    public ContainerBuilder Background(string hex) => parent.Background(hex);
}
